...
public class ReadToolCalling {
    ...
    @Autowired
    public ReadToolCalling(ContractEventService contractEventService,
                           CustomerEventService customerEventService,
                           @Qualifier("mainContractService")ContractService contractService,
                           @Qualifier("validatingCustomerService")CustomerService customerService) {
    ...
    @Tool(description = "Retrieve all contract events for a specific contract. A contract event is a an event log entity.")
    ...
    @Tool(description = "Retrieve all customer events for a specific customer. A customer event is a an event log entity.")
    public String getCustomerEvents(UUID customerId) {
        try {
            System.out.println("AI try to receive contract events for contract " + customerId);
            List<CustomerEventDto> list = customerEventService.getEventsForCustomer(customerId);
            System.out.println("List of CustomerEventDto: " + list.toArray().length);
            return "Successfully found these events: " + list;
        } catch (Exception ex) {
            return "Error by searching due to " +  ex.getMessage();
        }
